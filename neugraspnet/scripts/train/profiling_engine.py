...
class Engine(Serializable):
    ...
    def _run_once_on_dataset(self) -> float:
        ...
        try:
            ...
            while True:
                if ((self.state.iteration >= 25) and (self.state.iteration <= 100)):
                    with profiler.profile(with_stack=True, profile_memory=True) as prof:
                        try:
                            ...
                            # prof.enable()
                            with profiler.record_function("DATALOADER NEXT BATCH"):
                        ...
                        with profiler.record_function("TRAIN BATCH"):
                            self.state.output = self._process_function(self, self.state.batch)
                        self._fire_event(Events.ITERATION_COMPLETED)
                    import os
                    os.makedirs(str(self.logdir)+"/profiler",exist_ok=True)
                    print(prof.key_averages(group_by_stack_n=5).table(sort_by='cpu_time_total', row_limit=10),flush=True)
                    prof.export_chrome_trace(str(self.logdir)+"/profiler/trace"+str(self.state.iteration)+".json")
                    # import pdb; pdb.set_trace()
                    # prof.disable()
                    # stats = pstats.Stats(prof).strip_dirs().sort_stats("cumtime")
                    # stats.print_stats(15) # top 15rows
